...
export default function NoteEditor({
  ...
  // Загружаем контент только при смене заметки (не на каждый рендер)
  useEffect(() => {
  ...
  // Чекбоксы внутри contentEditable
    ...
    >
      ...
      <header className="glass sticky top-0 z-10 flex items-center gap-1 border-b border-ink-100 dark:border-ink-800 px-3 py-2">
        ...
        <div className="ml-auto flex items-center gap-0.5">
          ...
          >
            <Pin
              className={cn("size-4.5 -rotate-45", note.pinned && "text-accent-600")}
          ...
          >
            <Star
              className={cn("size-4.5", note.favorite && "text-amber-500")}
          ...
          <ToolBtn onClick={share} label="Поделиться">
          ...
          <ToolBtn
            onClick={() => {
              onDelete();
              onToast("Заметка перемещена в корзину 🗑");
            }}
            label="В корзину"
          >
        ...
        <div className="mx-auto max-w-3xl px-5 py-6 sm:px-8">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {note.tags.map((t) => (
              <span
                key={t}
                className="inline-flex items-center gap-1 rounded-full bg-accent-100 dark:bg-accent-600/20 px-2.5 py-1 text-xs font-semibold text-accent-700 dark:text-accent-300"
              >
                #{t}
                <button
          ...
          <div
            className="sticky top-0 z-[5] mt-4 flex flex-wrap items-center gap-0.5 rounded-2xl border border-ink-100 dark:border-ink-800 bg-white/90 dark:bg-ink-900/90 backdrop-blur p-1 shadow-sm"
            role="toolbar"
            aria-label="Форматирование"
          >
            <ToolBtn onClick={() => exec("bold")} label="Жирный (Ctrl+B)">
              <Bold className="size-4" />
            </ToolBtn>
            <ToolBtn onClick={() => exec("italic")} label="Курсив (Ctrl+I)">
              <Italic className="size-4" />
            </ToolBtn>
            <ToolBtn onClick={() => exec("underline")} label="Подчёркнутый (Ctrl+U)">
              <Underline className="size-4" />
            </ToolBtn>
            <span className="mx-1 h-5 w-px bg-ink-200 dark:bg-ink-700" />
            <ToolBtn onClick={() => exec("formatBlock", "h1")} label="Заголовок 1">
              <Heading1 className="size-4" />
            </ToolBtn>
            <ToolBtn onClick={() => exec("formatBlock", "h2")} label="Заголовок 2">
              <Heading2 className="size-4" />
            </ToolBtn>
            <ToolBtn onClick={() => exec("formatBlock", "blockquote")} label="Цитата">
            ...
            <ToolBtn onClick={insertChecklist} label="Чек-лист">
            ...
            <div className="mt-4 space-y-2.5">
              {note.audios.map((clip) => (
                <AudioPlayer
                  key={clip.id}
                  clip={clip}
                  onDelete={() => onRemoveAudio(clip.id)}
                />
              ))}
            </div>
          )}
